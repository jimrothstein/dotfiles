...
### TODO
- 138 near-duplicate candidate files remain across the 75 clusters below (18 clusters had ≥1 member removed on 2026-08-19; each still keeps ≥1 file, so all 75 clusters remain listed). Check `[X]` to delete a file, `[ ]` = keep. Clusters still fully `[ ]` = pending user review.
...
## NEXT STEPS
- User continues reviewing the remaining near-duplicate clusters in TODO (still all `[ ]`) and marks `[X]` on files to delete. Then we move the checked files (same policy: recoverable staging in `/tmp/opencode/`).
- Staged near-duplicates in `/tmp/opencode/near_dup_removed_2026-08-19/` (23 files / 72.6 MB) can be hard-deleted once satisfied (`rm -rf`). As with the earlier staging dir, `/tmp` may be cleared at any time — decide before assuming recovery is possible.
