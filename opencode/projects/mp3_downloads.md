...
- 2026-09-11: Full project review (no changes made, report only). Directory now 4,923 entries / ~20 GB.
  - **Aug 2026 batch near-duplicates (NEW since last scan):** 65 raw `title [videoID].ogg` downloads (Aug 9–10, ~899 MB) coexist with 55 re-encoded `NA_title_DATED.ogg` files (Aug 8–10, ~251 MB). Word-similarity matching found 53 matched pairs (same content, different re-encode → near-dups, ~1.1 GB duplicated). The 10AUG2026 NA_ superset also includes 2 files with no surviving `[id]` twin (Calculus book = archive id gt99u_zKpi8, Yiddish song = archive id huSD6GysL6k — those two ids have no file on disk). No byte-identical dups among this batch.
  - **17 true-byte-identical duplicate clusters** (36 files, ~50.4 MB reclaimable, MD5-verified) freshly found that prior scans missed — all 2022-era files where canonicalization differs only by `&`/`and`, apostrophe, or `.ogg`-extension-missing. Examples: `Ann_Margret_Elvis_Presley_&_Ann_Margret_Your_The_Boss_22JAN2019.ogg` ×2; `Bobby_Rydell_Ann_Margret_One_Boy_(1963)` in 3 byte-identical copies incl. one without `.ogg` ext; `Bob_Wills_Keep_Knockin_...14Oct2020` (+ no-ext twin); `Herman_s_Hermits / Herman's Hermits_There_s_a_Kind_of_Hush.mp3` ×2. NOT deleted — await user decision.
  - **3 incomplete downloads** (`.webm.part`): `Cindy_Malone_...23Aug2022` (3.7 MB), `Focus_Music_...2021-09-08` (55 MB), `NA_Westinghouse Full Feature Documentary_08Aug2026` (65 MB, from Aug 8).
  - **AGENTS.md symlink is broken** — points to `/home/jim/code/docs/AGENTS/mp3_AGENTS.md` which does not exist.
  - Git: clean, single commit (Aug 16 scaffolding). `duplicate_report.md` is historical.
- 2026-09-11: Completed Aug 2026 near-dup pass — ALL 53 `NA_`/`[videoID]` pairs processed group-by-group (A = `NA_...` re-encode, B = raw `... [videoID].ogg`). Decisions: **delete A** (40 groups), **delete both** (6 groups: 1, 20, 21, 27, 39, 47), **keep both** (7 groups: 2, 12, 23, 24, 37, 38, 51). 52 files total moved (recoverable staging, per-group dirs `/tmp/opencode/dup_2026-09-11_gN/`, sizes verified after each move); raw `[id]` files kept wherever stated. Only 9 `NA_` files remain in archive (7 from keep-both groups + the 2 unmatched: Calculus book = id gt99u_zKpi8, Yiddish song = id huSD6GysL6k). Note: several filenames use fullwidth/curly Unicode characters (＂”, ’， •, ＊, ⧸) — `mv` failed when using ASCII replacements; must use exact chars (check with `ls`).
...
## NEXT STEPS
- [COMPLETED 2026-09-11] Aug 2026 near-dup pass: all 53 `NA_`/`[videoID]` pairs processed (40 delete-A, 6 delete-both, 7 keep-both); 52 files / 323 MB staged in `/tmp/opencode/dup_2026-09-11_gN/`; 9 NA_ .ogg remain. Directory 4923 → 4871.
- Decide on the 17 new true-byte-identical clusters (50.4 MB) found 2026-09-11 — same staging-policy deletion if approved.
- Clean up 3 `.webm.part` incomplete downloads and fix the broken AGENTS.md symlink (/home/jim/code/docs/AGENTS/mp3_AGENTS.md missing).
